...
def gpx2database(gpxfile,wteapi_key,Session,infomarker,track,trackpoint,timezone,tz_detail):
    ...
    query=session.query(track).filter(and_(track.date==trkpt_1ts,track.trkptnum==trk_ptnum,track.distance==trk_distance,track.timespan==trk_span))
    if query.count() == 1:
	for detail in query.all():
	    track_detail=detail
	...
	for detail in query.all():
        ...
        session.add(track(trkpt_1ts,trk_ptnum,trk_distance,trk_span))
	session.commit()
    	for detail in query.all():
	    track_detail=detail
